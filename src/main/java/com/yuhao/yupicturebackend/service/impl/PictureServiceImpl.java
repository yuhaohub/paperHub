...
import com.yuhao.yupicturebackend.untils.ColorSimilarUtils;
...
public class PictureServiceImpl extends ServiceImpl<PictureMapper, Picture>
    ...
    public void fillReviewParams(Picture picture, User loginUser) {
        ...
        if (userService.isAdmin(loginUser)) {
            ...
            picture.setReviewTime(currentDate);
            return;
        }
        ...
        if (space.getSpaceType() == SpaceTypeEnum.PRIVATE.getValue()) {
            // **私人空间：仅空间创建者本人上传的图片自动通过**
            if (space.getUserId().equals(loginUser.getId())) {
                picture.setReviewStatus(PictureReviewStatusEnum.PASS.getValue());
                picture.setReviewerId(loginUser.getId());
                picture.setReviewMessage("私人空间图片自动过审");
                picture.setReviewTime(currentDate);
                return;
            }
        }
//        else if (space.getSpaceType() == SpaceTypeEnum.TEAM.getValue()) {
//            // **团队空间：管理员或有审核权限的用户自动通过**
//            if (teamService.isTeamAdmin(spaceId, loginUser.getId()) || teamService.hasReviewPermission(spaceId, loginUser.getId())) {
//                picture.setReviewStatus(PictureReviewStatusEnum.PASS.getValue());
//                picture.setReviewerId(loginUser.getId());
//                picture.setReviewMessage("团队管理员/有审核权限用户自动过审");
//                picture.setReviewTime(currentDate);
//                return;
//            }
//        }
